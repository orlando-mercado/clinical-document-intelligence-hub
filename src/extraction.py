...
enforces the JSON shape returned; src.llm.call_structured handles the
retry loop for what structured outputs does *not* enforce server-side
(numeric bounds like `confidence` being in [0, 1] are validated
client-side by Pydantic — see src/llm.py) plus transient API errors and
safety refusals.
...
DOC_LABEL: dict[DocumentType, str] = {
...
def extract(
    ...
    *,
    ...
    doc_label = DOC_LABEL[document_type]
    ...
    try:
        result = call_structured(
            ...
            label=f"extraction of {doc_label} from '{document.source_filename}'",
        )
